...
* **再構築されたプレゼンテーション** `<名前>.repaired.pptx` — 生存データにスライドが残っている場合（末尾切断型・バージョン混在型・中間破損型）。生存していた全スライドが復元されます。破損とともに画像が失われた場合、再構築後のファイルは開けますが、失われた画像への参照が残るため、初回起動時にPowerPointが一度だけ修復を提案することがあります（次リリースでこの残存参照の自動除去に対応予定）。
