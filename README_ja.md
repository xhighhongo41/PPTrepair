...
* **`--search-archives` でバックアップアーカイブも復元素材の探索対象になります。** 走査中に見つかったzip/tar系アーカイブ（`.zip`, `.tar`, `.tar.gz`/`.tgz`, `.tar.bz2`/`.tbz2`, `.tar.xz`/`.txz`）を開き、中の `.pptx`/`.pptm` をすべて診断します — バックアップの中に眠っていた無傷の双子や旧バージョンが、`backup.zip::内部/deck.pptx` というラベルで復元候補・別バージョン候補・マージ候補に現れます。アーカイブ内のファイルはあくまで素材であり、修復対象にもスキャン件数・破損統計にも数えられません。メンバーは1つずつ一時ディレクトリに展開され、終了後に削除されます。クラウド専用のアーカイブはPowerPointファイルと同じ規則に従います（`--allow-download` なしではスキップ）。フラグ指定時のレポートJSONには `schema_version: 4` とアーカイブ由来候補への `origin_archive` キーが付き、未指定時の出力は従来と完全に同一です。
...
第2引数以降にはバックアップアーカイブ（`.zip`, `.tar`, `.tar.gz`/`.tgz`, `.tar.bz2`/`.tbz2`, `.tar.xz`/`.txz`）をそのまま指定することもできます。中の `.pptx`/`.pptm` は一時ディレクトリに展開されたうえで通常のファイルとまったく同じ審査を受け、確認ダイアログとレポートでは `backup.zip::内部/deck.pptx` というラベルで表示されます（`--json` では各供給元に `origin_archive` が記録されます）。暗号化・破損・読み取り不能なメンバーはnote付きでスキップされます。復元対象（第1引数）自体をアーカイブ内のファイルにすることはできません——先に展開してください。
...
* **Multi-source** — `merge`と同様、他のコピーやドロップしたバックアップアーカイブから採掘したドナー素材を使って修復します。修復前にドナー承認ダイアログが各候補の根拠を提示し、チェックのオン/オフで採否を選べます。検証済みのバイト完全一致ドナーは既定でチェック済み、根拠の弱いcandidate/lineage層の候補は未チェックの状態から始まり、明示的な承認が必要です（CLIの信頼モデルと同じです）。
