...
* 隠しファイル——macOSのAppleDoubleメタデータ `._foo.pptx` のような `.` で始まる名前——は既定でスキップして件数集計します。ディスク上のファイルにも `--search-archives` で探索するアーカイブ内のメンバーにも同じ規則が適用されます。`--include-hidden` を付ければ従来どおり検査対象になります。
...
* 隠しファイル（`.` で始まる名前）は `scan` と同じく既定でスキップして件数集計します。`--include-hidden` を付ければ従来どおり対象になります。
...
出力先は各ファイルの隣（**in-place**）、または入力ツリーをミラーする指定フォルダのいずれかを選べます——それぞれCLIの`--in-place`と集約`-o OUTDIR`に相当します。環境設定ダイアログでは、クラウド専用ファイルのダウンロード可否、隠しファイル（macOSの `._*` メタデータのような `.` で始まる名前）を無視するか（既定ON。CLIで `--include-hidden` を付けない状態に相当）、検査する1ファイルの上限サイズ（既定2GB。`--max-file-size`に相当）、UI・レポート言語（CLIと同じ7言語。変更は再起動後に反映されます）を設定でき、これらの設定はすべて自動保存され、次回起動時にも引き継がれます。CLIと同様、ドロップしたアーカイブ・Evernoteのエクスポートはあくまでドナー素材であり、その中のファイル自体が修復対象になることはありません。
...
- rejectされたソースには理由（見つからない／アクセスできない／非対応形式）が詳細ダイアログに表示されるようになりました。ネットワーク（SMB）マウント上のファイルが誤rejectされる不具合を修正: ドロップされたパスとディスク上の実名のNFC/NFD正規化不一致（濁点・半濁点を含む日本語名など）はディレクトリ一覧から実表記を復元し、どの表記でも`stat`が失敗する場合は`tar`と同じopenベースの判定にフォールバックし、その他の一時的な失敗は1回リトライ後に名前ベースで受理します
...
- ネットワーク（SMB）マウント越しの長時間スキャン・一括修復が、マウントの一時的な不調で停止しなくなりました——実物の677GBバックアップで実証済みの堅牢化です。アーカイブの読み取りが2GiBを超えると `OverflowError` で停止する不具合を修正（バイトカウンタがどんなサイズでも溢れず、進捗％も正確なまま）。アーカイブの**オープン時**の一時的な失敗は数秒のリトライで自然回復し、読み取り途中のエラーはnoteに降格して抽出済みメンバーを保持します。そして実際に観測された最悪ケース——全読み取り成功**後**の最後のcloseだけが失敗するSMBのstaleハンドル——でも、完成した成果物はもう破棄されません: 採掘済みドナー素材は「closing archive failed (extracted data intact)」のnote付きでそのまま使われ、一括修復は残りのソースで続行します
- 隠しファイル（`.` で始まる名前——macOSのAppleDoubleメタデータ `._foo.pptx`、zip内の `__MACOSX/` エントリ、インポーターの残骸など）を、ディスク上でもアーカイブ内でも既定でスキップするようになりました。プレゼンでないファイルが「修復不能」件数を水増しすることがなくなります。件数は独立に集計されます（GUIでは「スキップ(隠しファイル)」、レポートでは「スキップ: 隠しファイル N 件」）。GUIのチェックボックス（既定ON）または `scan`/`repair-all` の新フラグ `--include-hidden` で従来挙動に戻せます。GUIでセッション中に切り替えてもアーカイブのセッションキャッシュが再利用されるため、巨大アーカイブの再読み込みは発生しません
