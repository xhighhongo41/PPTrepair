...
構造上無傷のファイルに対しては、`check` はさらにパッケージ内のXML関係参照を検証し、未解決の参照（初回起動時にPowerPointが一度だけ修復を提案する原因）があれば報告します。判定と終了コードは変わりません（`--json` では `xml_ref_integrity` に詳細が入ります）。PPTrepair 1.1.1以前の修復で残ることのあった残存参照もこれで検出できます。
...
- 再構築後のファイルがそのまま開けるようになりました: 破損とともに画像・動画が失われた場合、`repair` がスライドXML内の宙吊り参照を自動的に除去するため、初回起動時にPowerPointが修復を提案しなくなります。動画の実体が失われた場合はポスターフレームが静止画として残ります
- `check` に参照整合検査を追加: 構造上無傷のファイルの未解決関係参照を報告します（件数と対象パート。`--json` では `xml_ref_integrity`）。判定・終了コードは変わりません。過去バージョンの修復で残った残存参照もこれで検出できます
- `repair` は生成した成果物を自己検証し、修復後に残る未解決参照数を報告します。trim成果物は元アーカイブとバイト単位で同一のため、元から存在した未解決参照は報告のみ行い変更しません
