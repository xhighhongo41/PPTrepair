...
- Rejected sources now show why in the details dialog (not found / inaccessible / unsupported format). Fixed a bug where files on a network/VPN-mounted volume could be misclassified as unsupported after a transient `stat` failure (one retry, then name-based acceptance)
