...
* **`--search-archives` also mines backup archives — and Evernote exports — for restore material.** Zip archives, tar archives (`.zip`, `.tar`, `.tar.gz`/`.tgz`, `.tar.bz2`/`.tbz2`, `.tar.xz`/`.txz`) and Evernote exports (`.enex`) found during the walk are opened and every `.pptx`/`.pptm` inside — a note's PowerPoint attachment, for an `.enex` export — is diagnosed; an intact twin or an older version of a corrupted file kept inside a backup then shows up among the restore/lineage/merge candidates, labelled `backup.zip::inner/deck.pptx`. Archived files are donor material only: they are never repaired, never counted as scanned or corrupted, and members are extracted one at a time to a temporary directory that is removed afterwards (tar archives and `.enex` exports are read in a single streaming pass, however many members/attachments they hold). Cloud-only archives follow the same rule as cloud-only PowerPoint files (skipped unless `--allow-download`). With the flag, the report JSON carries `schema_version: 4` and an `origin_archive` key on archive-derived candidates; without it, output is unchanged.
...
* Hidden files — names starting with `.`, such as macOS AppleDouble `._foo.pptx` metadata — are skipped and counted by default, on disk and inside archives mined with `--search-archives` alike; pass `--include-hidden` to examine them too.
...
* Hidden files (names starting with `.`) are skipped and counted by default, exactly as in `scan`; pass `--include-hidden` to examine them too.
...
Any SRC after the first may also be a backup archive or an Evernote export (`.zip`, `.tar`, `.tar.gz`/`.tgz`, `.tar.bz2`/`.tbz2`, `.tar.xz`/`.txz`, `.enex`): every `.pptx`/`.pptm` inside — a note's PowerPoint attachment, for an `.enex` export — is extracted to a temporary directory and vetted exactly like a plain file, and is named `backup.zip::inner/deck.pptx` in prompts and reports (`--json` additionally records each source's `origin_archive`). Encrypted, damaged or unreadable members are skipped with a note. The file being restored itself cannot live inside an archive — extract it first.
...
Drop files, folders (scanned recursively for `.pptx`/`.pptm`), backup archives or Evernote exports (`.enex`) onto the window — repeated drops keep accumulating into one work set. **Scan** diagnoses everything with live progress and a Cancel button, then lists the results in a **Files** tab (colour-coded verdicts) and a **Candidates** tab (the same restore/lineage/merge candidates `scan --report` writes to disk). Archive material mined while scanning is cached for the rest of the session — so **Repair** never re-reads the archives themselves, and the cache is discarded when the app closes — while reading a large archive shows progress as a percentage in the status bar. **Repair** then runs in one of two modes:

* **Single-file** repairs each corrupted file from its own bytes only, like `repair`/`repair-all`.
* **Multi-source** merges in donor material mined from other copies and from any dropped backup archives or Evernote exports, like `merge` — a donor-approval dialog shows the evidence for every candidate and lets you check or uncheck it before repairing; verified byte-identical donors are pre-checked, while the weaker candidate/lineage matches start unchecked and need deliberate approval, mirroring the CLI's own trust model.

Output goes either **in place** next to each source, or into a chosen folder that mirrors the input tree — the GUI equivalents of `--in-place` and aggregate `-o OUTDIR`. A Preferences dialog controls whether cloud-only files may be downloaded, whether hidden files (dot-prefixed names such as macOS `._*` metadata) are ignored (on by default, the GUI equivalent of leaving `--include-hidden` off), the maximum file size examined (default 2 GB, the GUI equivalent of `--max-file-size`), and the UI/report language — the same 7 languages as the CLI, with a change taking effect after restart. Every setting persists across launches. As with the CLI, dropped archives and Evernote exports are only donor material: the files inside them are never repaired themselves.
...
### ver 2.2.0 (2026-08-01)
- Evernote exports (`.enex`) are now accepted as a donor container everywhere backup archives are — GUI drag & drop, folder scan (`--search-archives`), and `merge`'s SRC arguments all pick them up automatically. PowerPoint attachments (`.pptx`/`.pptm`) are streamed out of the export as it is parsed, and an attachment whose own head is damaged is still kept as donor material (useful for `merge`)
- Tar-family archives are now mined in a single streaming pass instead of decompressing the whole archive once per member, making very large backups practical to search. In the GUI, archive material mined during **Scan** is cached for the session and not re-read during **Repair**; reading progress is shown as a percentage in the status bar and can be cancelled like the rest of the scan
- Rejected sources now show why in the details dialog (not found / inaccessible / unsupported format). Fixed a bug where files on network (SMB) mounts could be wrongly rejected: an NFC/NFD Unicode-normalization mismatch between the dropped path and the on-disk name (any Japanese name with a dakuten, for instance) is now recovered from the directory's own listing, a `stat` that fails under every spelling falls back to the same open-based probe `tar` uses, and other transient failures get one retry followed by name-based acceptance
- The GUI's discovery phase now shows which directory is currently being walked as soon as a scan starts, instead of staying silent until the first file is diagnosed, and stays cancellable throughout
- Sources that live on different physical disks (mounts) are now scanned in parallel — up to four device groups at a time, while each single disk is still read sequentially — so a slow network mount no longer stalls the scan of the local disks
- The donor-approval dialog gained three bulk-action buttons: Select all, Deselect all, and Reset to default
- Long scans and batch repairs over network (SMB) mounts no longer die of the mount's bad moments — hardening proven on a real 677 GB backup. Reading an archive past 2 GiB no longer stops with an `OverflowError` (byte counters survive any archive size and the progress percentage stays correct); a transient failure while *opening* an archive is retried for a few seconds before giving up; a read error mid-sweep degrades to a note and the members already extracted are kept; and — the failure actually observed in the wild — a stale SMB file handle whose final close fails *after* every read succeeded no longer throws the completed work away: the mined donor material is kept (noted as "closing archive failed (extracted data intact)") and a batch repair continues with its remaining sources instead of stopping
- Hidden files (names starting with `.` — macOS AppleDouble `._foo.pptx` metadata, `__MACOSX/` entries inside zips, importer debris) are now skipped by default, both on disk and inside archives, so they no longer inflate the unrepairable count with files that were never presentations. They are counted separately (`skipped (hidden)` in the GUI, `Skipped: N hidden file(s)` in reports); a GUI checkbox (on by default) and a new `--include-hidden` flag for `scan`/`repair-all` restore the old behaviour. Toggling the GUI option mid-session replays the session's archive cache, so a huge archive is never re-read for it
